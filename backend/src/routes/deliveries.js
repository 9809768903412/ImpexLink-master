...
function getGpsDeviceToken() {
  return cleanEnv(
    process.env.GPS_DEVICE_TOKEN || process.env.DELIVERY_GPS_TOKEN || "",
  );
}
...
function getRequestGpsToken(req) {
  ...
  return String(
    req.headers["x-gps-token"] || req.body?.token || req.query?.token || "",
  ).trim();
}
...
function normalizeGpsLocation(row) {
  ...
  return {
    ...
    speedKmph:
      row.speedKmph === null || row.speedKmph === undefined
        ? null
        : Number(row.speedKmph),
    heading:
      row.heading === null || row.heading === undefined
        ? null
        : Number(row.heading),
    satellites:
      row.satellites === null || row.satellites === undefined
        ? null
...
async function assertDeliveryVisible(req, deliveryId) {
  ...
  return prisma.delivery.findFirst({
    where: {
      AND: [{ deliveryId: Number(deliveryId), deletedAt: null }, scopeWhere],
    },
...
async function buildDeliveryScope(req) {
  ...
  if (roleList.includes("PROJECT_MANAGER")) {
    scopes.push({
      clientOrder: { project: { assignedPmId: req.user.userId } },
    });
  }
...
function mapDelivery(d) {
  return {
    ...
    receiverAddress:
      d.receiverAddress || d.clientOrder?.client?.address || null,
...
router.post("/active/location", async (req, res, next) => {
  try {
    ...
    if (getRequestGpsToken(req) !== configuredToken) {
      return res.status(401).json({
        error: "Invalid GPS device token",
      });
    }
  ...
  async (req, res, next) => {
    try {
      ...
      const status = req.query.status
        ? String(req.query.status).toUpperCase().replace(/-/g, "_")
        : "";
      ...
      const where = {
        AND: [
          ...
          status ? { status } : {},
          clientId &&
          (roleList.includes("ADMIN") || roleList.includes("WAREHOUSE_STAFF"))
          ...
          ...(columnSupport.batches
            ? {
                deliveryMethod: deliveryMethod
                  ? String(deliveryMethod).toUpperCase()
                  : thirdPartyProvider
                    ? "LALAMOVE"
                    : "TRUCK",
                loadKg:
                  loadKg === undefined || loadKg === "" ? null : Number(loadKg),
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      if (
        req.body.receivedBy !== undefined &&
        !isNonEmptyString(req.body.receivedBy)
      ) {
      ...
      if (requestedStatus) {
        ...
        if (
          ...
          return res
            .status(400)
            .json({ error: "Return rejection reason is required" });
        }
      }
          ...
          ...(columnSupport.batches
            ? {
                deliveryMethod: req.body.deliveryMethod
                  ? String(req.body.deliveryMethod).toUpperCase()
                  : undefined,
                loadKg:
                  req.body.loadKg === undefined || req.body.loadKg === ""
                    ? undefined
                    : Number(req.body.loadKg),
...
router.post("/:id/confirm", requireRole(["CLIENT"]), async (req, res, next) => {
  try {
    ...
    if (delivery.status !== "IN_TRANSIT" && delivery.status !== "DELAYED") {
      return res
        .status(400)
        .json({ error: "Only active deliveries can be confirmed." });
    }
    const access = await resolveClientAccess(prisma, req.user.userId);
    const ownsByClient =
      access?.client &&
      delivery.clientOrder?.clientId === access.client.clientId;
    const ownsByCreator = delivery.clientOrder?.createdBy === req.user.userId;
    if (!ownsByClient || (access?.isUserScoped && !ownsByCreator))
